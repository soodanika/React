...
export const withPrmomotedLabel = (ResturandCard) => {
    return (props) => {
        return (
            <div>
                <label className="bg-custom-yellow text-white absolute rounded-lg mx-3 p-1">{props.data.info.aggregatedDiscountInfoV3.header} {props.data.info.aggregatedDiscountInfoV3.subHeader}</label>
                <ResturandCard {...props} />
            </div>
        )
    }
